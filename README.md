# Web Component demo of custom card
...
Render the card where you need it like this

```html
<custom-card headline="Sample headine"> {...card content} </custom-card>
```

You can change the color of the headline, using HTML color names or any other valid color string

```html
<custom-card headline="Red headine" headlineColor="red">
  {...card content}
</custom-card>
```

Show a button in a card by setting `showButton={true}` in the component, each card instance will listen to button clicks.
